  ...
  module Webhooks
    module Trigger
      ...
      def perform_request
        ...
        if assistant_webhook_to_citadel?
          headers['X-Api-Key'] = ENV.fetch('CITADEL_API_KEY', nil)
          Rails.logger.info "[Citadel Webhook] Sending to #{@url} with API key, event: #{@payload[:event]}"
        end
        ...
        Rails.logger.info "[Citadel Webhook] Response: #{response.code}" if assistant_webhook_to_citadel?
        response
      end
